...
timestamps are Unix epoch seconds (matching ``ledger.py``'s ``state.db``).
...
CHAIN_GENESIS = "ledger-usage-chain/v1"
...
CREATE TABLE IF NOT EXISTS api_keys (
    ...
    org_id       TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    name         TEXT,
    prefix       TEXT NOT NULL,           -- shown in the UI, e.g. ledger_sk_AbC1
...
def init_schema(conn: sqlite3.Connection) -> None:
    # Forward-compat guard: refuse a database written by a NEWER Ledger rather
    ...
    if existing is not None and existing > SCHEMA_VERSION:
        raise RuntimeError(
            f"database schema_version {existing} is newer than this Ledger "
...
def create_api_key(conn, org_id: str, name: Optional[str] = None) -> tuple[sqlite3.Row, str]:
    ...
    kid = new_id("key")
    prefix = secret[:len(API_KEY_PREFIX) + 4]   # e.g. "ledger_sk_AbC1"
    conn.execute(
...
def record_ingest_health(conn, org_id: str, source: str,
    ...
    (e.g. ``"claude_code_hook"``, ``"api_key:ledger_sk_AbC1"``).
