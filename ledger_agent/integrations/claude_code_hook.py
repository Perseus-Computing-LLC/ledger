"""Claude Code / Codex CLI → Ledger, as a packaged Stop hook.
...
it into Ledger, so every coding turn shows up live on the dashboard and depletes
prepaid credit.

Wire it with ``ledger install-claude-hook`` (which merges the entry below into
``~/.claude/settings.json``), or by hand:

    {"hooks": {"Stop": [{"hooks": [
      {"type": "command",
       "command": "python -m ledger_agent.integrations.claude_code_hook"}
    ]}]}}

The org is ``$LEDGER_ORG`` (default "Claude Code"); the workspace is the basename
...
def meter_payload(payload: dict):
    ...
    meter = Meter(org=os.environ.get("LEDGER_ORG", "Claude Code"), tier="pro")
    try:
        res = meter.track(
...
def main(argv=None):
    ...
    try:
        ...
        return 0
    for a in res.alerts:
        sys.stderr.write(f"[ledger] {a['kind']}: {a['message']}\n")
    return 0
