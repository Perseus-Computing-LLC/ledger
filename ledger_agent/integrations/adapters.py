"""Provider/framework adapters — normalize usage objects → Ledger meter calls."""
...
def track_anthropic(meter, response: Any, *, model: Optional[str] = None,
    ...
    """Meter an Anthropic Messages API response (``response.usage``).
        ...
        from ledger_agent.integrations import track_anthropic
...
def track_hermes_session(meter, session: dict, *, workspace: Optional[str] = None):
    ...
    - a raw ``sessions`` row (``actual_cost_usd`` / ``estimated_cost_usd``), the
      format the original ``ledger.py`` reads; or
    - a per-model event from :func:`ledger_agent.hermes.read_spend_events`,
