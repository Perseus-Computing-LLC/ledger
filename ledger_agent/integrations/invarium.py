"""Invarium → Ledger bridge: accuracy-gated savings metering.
...
the-tool", …). Ledger meters spend per call and books *savings* only when a
counterfactual ``baseline_cost_usd`` is supplied.

This bridge makes the savings figure **accuracy-gated**: the counterfactual
baseline is forwarded to Ledger *only when the task passed its Invarium
contract*. A cheaper-but-wrong run therefore books **$0** savings.

Why this needs no schema change: Ledger already treats a withheld (NULL) baseline
...
``metadata: dict`` works — so ``ledger_agent`` takes **no** dependency on invarium.
"""
...
BASELINE_MODEL_KEY = "baseline_model"   # or a model name Ledger prices itself
...
def meter_agent_result(
    ...
    **record_kwargs: Any,
    ...
    Returns the :class:`ledger_agent.metering.MeterResult` unchanged so callers
