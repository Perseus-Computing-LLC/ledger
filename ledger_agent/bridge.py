"""Bridge to the live runway monitor (repo-root ``ledger.py``).
...
engine shells out to it for ``--json`` exactly as ``ledger_route.py`` does. If
the monitor isn't configured or fails, the dashboard simply omits the live
provider-runway panel — the rest of Ledger works fully offline.
"""
...
def runway(monitor_cfg: dict) -> dict | None:
    ...
    try:
        ...
        sys.stderr.write(f"ledger: monitor command unparseable: {e}\n")
        return None
    if not argv:
        return None
    binary = argv[0]
    ...
    if not os.path.isabs(binary):
        sys.stderr.write(
            "ledger: monitor command must be an absolute path "
            f"(got {binary!r}); refusing to run\n")
        return None
    if binary not in allowed:
        sys.stderr.write(
            f"ledger: monitor binary {binary!r} not in monitor.allowed_binaries; "
            "refusing to run\n")
        return None
    try:
        ...
        sys.stderr.write(f"ledger: monitor bridge unavailable: {e}\n")
        return None
