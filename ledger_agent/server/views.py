...
def render_dashboard(summary: dict, *, orgs: list, cfg: dict,
    ...
    if integrity is not None:
        ...
        if o is None or o["status"] == "empty":
            ...
            integrity_card = (
                ...
                "<div class='s'>hash chain broken — run <span class='num'>ledger verify</span></div></div>")
    ...
    if integrity is not None:
        if checkpoint:
            ...
            checkpoint_card = (
                ...
                "<span class='num'>ledger verify-checkpoints --file</span> "
    ...
    # is only a voluntary thank-you path after Ledger can show verified savings.
    can_checkout = stripe_status["available"]
    sb = stripe_status["mode"]
    if can_checkout:
        billing = """
        <div class="billing">
          <span class="muted">Free covers teams of up to 10 people. When Ledger can verify savings,
    ...
    # ATTRIBUTION: Ledger MEASURES; it does not save. Perseus (routing) + Vault
    ...
    # verified by Ledger"). Standalone (no baseline) we show spend + a flagship-
    ...
    if eff.get("events"):
        ...
        if has_savings:
            ...
            num = _usd(gross)
            sub = f"this month · {_e(mult_s)} efficiency · verified by Ledger"
    ...
    if runway and runway.get("providers"):
        ...
        runway_panel = f"""
        <div class="panel">
          <h2 class="section-title">Provider runway <span class="hint">live, via ledger.py monitor</span></h2>
    ...
    curl = (f"curl -X POST {base_url}/v1/usage \\\n"
            f"  -H 'Authorization: Bearer ledger_sk_…' \\\n"
...
def pricing_page(*, stripe_status: dict, org_id: str | None = None,
    ...
    for key in pricing.TIER_ORDER:
        ...
        if key == "pro":
            ...
            cta = ("<a class='btn ghost' href='mailto:tcconnally@gmail.com?"
                   "subject=Ledger%20Team'>Talk to us →</a>")
        else:
            cta = ("<a class='btn ghost' href='mailto:tcconnally@gmail.com?"
                   "subject=Ledger%20Enterprise'>Contact sales</a>")
    ...
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Ledger — Pricing</title>{FAVICON}
<style>{CSS}
...
<div class="top"><div class="brand"><div class="logo">◆</div>
  <div><h1>Ledger</h1><div class="tag">Plans &amp; pricing</div></div></div>
...
def api_key_created_page(secret: str, base_url: str) -> str:
    ...
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Ledger — API key created</title>{FAVICON}
<style>{CSS}</style></head><body><div class="wrap" style="max-width:680px">
<div class="brand" style="margin-bottom:20px"><div class="logo">◆</div><div><h1>Ledger</h1></div></div>
<div class="panel" style="padding:26px 22px">
  ...
  <div class="muted" style="margin-top:8px">Copy it now — for your security, Ledger stores only a hash and
...
def login_page(login_href: str) -> str:
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Ledger — Sign in</title>{FAVICON}
...
def simple_page(title: str, heading: str, body_html: str, *, ok: bool = True) -> str:
    ...
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Ledger — {_e(title)}</title>{FAVICON}
