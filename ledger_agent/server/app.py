...
class Handler(BaseHTTPRequestHandler):
    server_version = f"Ledger/{__version__}"
    ...
    def _log_exc(self, method, path, exc) -> str:
        ...
        sys.stderr.write(f"ledger: [{ref}] {method} {path} failed: {exc!r}\n")
        return ref
    ...
    def _dashboard(self, conn, q):
        ...
        if not org_id:
            return self._send(200, views.simple_page(
                "Ledger", "No organization yet",
                "Run <span class='num'>ledger init</span> to create one, or "
                "<span class='num'>ledger serve --demo</span> to explore with sample data.",
            ))
        ...
        # anchored — the tile nudges toward `ledger checkpoint`).
    ...
    def _bearer_org(self, conn):
        """Resolve the org from an ``Authorization: Bearer ledger_sk_…`` header."""
    ...
    def _checkpoints_get(self, conn, q):
        ...
        return self._json(200, {
            ...
            "note": "retain these out of band (email/S3 object-lock/git) — "
                    ...
                    "with `ledger verify-checkpoints --file <retained copy>`",
        })
    ...
    def _ingest_usage(self, conn):
        ...
        for ev in events:
            ...
            for field in ("policy_version", "result_hash", "human_review", "correction_ref",
                          "agent_id", "authority_manifest_ref",
                ...
                if idem_key and not db.claim_idempotency_key(
                    ...
                    for ev in events:
                        res = metering.record_usage(
                            ...
                            authority_manifest_custody=ev.get("authority_manifest_custody"),
    ...
    def _webhook(self, conn):
        ...
        try:
            ...
            sys.stderr.write(f"ledger: webhook rejected: {e!r}\n")
        ...
        sys.stderr.write(
            f"ledger: stripe event id={event.get('id', '')} type={event.get('type', '')}\n")
...
def _guard_insecure_bind(host, auth_on, cfg):
    ...
    allow_insecure = bool(cfg.get("server", {}).get("allow_insecure")) or \
        os.environ.get("LEDGER_ALLOW_INSECURE", "").strip().lower() in ("1", "true", "yes")
    if auth_on or loopback or allow_insecure:
        return
    raise SystemExit(
        f"ledger: refusing to serve on host {host!r} with authentication disabled; "
        "this would expose the billing dashboard and API to the network.\n"
        "  Do one of:\n"
        "   - enable auth (LEDGER_AUTH_ENABLED=1 + Google client id/secret + an https base_url)\n"
        "   - bind 127.0.0.1 behind a TLS-terminating reverse proxy\n"
        "   - for a trusted network only, pass --allow-insecure (LEDGER_ALLOW_INSECURE=1)")
...
def serve(host=None, port=None, db_path=None, demo=False, cfg=None,
    ...
    if importlib.util.find_spec("yaml") is None:
        raise SystemExit("ledger: PyYAML is required to run the server "
                         "(pip install 'ledger-agent[all]').")
    ...
    sys.stderr.write(
        f"\n  ◆ Ledger v{__version__} — the billing layer for AI agents\n"
        f"    dashboard:  {url}\n"
