...
STATE_COOKIE = "ledger_oauth_state"  # binds an in-flight OAuth login to the browser that began it (login-CSRF guard)
...
def handle_callback(conn, cfg, q: dict, client_ip: Optional[str] = None,
    ...
    the value of the ``ledger_oauth_state`` cookie set on this browser at
    ...
    user_id = _authorize_email(conn, cfg, email, name=claims.get("name"),
                               client_ip=client_ip)
    if not user_id:
        raise AuthError(f"{email} is not authorized to use this Ledger instance")
...
def csrf_token(session_token: str) -> str:
    ...
    import hashlib
    import hmac
    return hmac.new(session_token.encode("utf-8"), b"ledger-csrf-v1",
