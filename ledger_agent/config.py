"""Configuration — ``~/.ledger/config.yaml`` load / save / defaults.

Ledger follows the original monitor's philosophy: sensible defaults, everything
...
minimal built-in reader so ``ledger`` still runs.
"""
...
# ---------------------------------------------------------------- locations ---
def home_dir() -> Path:
    """The Ledger home (``~/.ledger`` unless ``LEDGER_HOME`` overrides)."""
    return Path(os.environ.get("LEDGER_HOME", str(Path.home() / ".ledger")))


def config_path() -> Path:
    return Path(os.environ.get("LEDGER_CONFIG", str(home_dir() / "config.yaml")))


def db_path() -> Path:
    return Path(os.environ.get("LEDGER_DB", str(home_dir() / "ledger.db")))


def chain_hmac_key(cfg: Optional[dict] = None) -> Optional[bytes]:
    """Resolve the ledger tamper-evidence HMAC key (#108) as bytes, or ``None``.

    Precedence: the ``LEDGER_CHAIN_HMAC_KEY`` env var, else ``ledger.hmac_key``
    from the loaded config. Empty/absent => ``None`` (plain SHA-256 chain).
    """
    val = os.environ.get("LEDGER_CHAIN_HMAC_KEY")
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "auth": {
        ...
        "enabled": False,
        "google_client_id": "",       # or env LEDGER_GOOGLE_CLIENT_ID
        "google_client_secret": "",   # or env LEDGER_GOOGLE_CLIENT_SECRET
        "base_url": "",               # public origin, e.g. https://ledger.perseus.observer
    ...
    "billing": {
        ...
        # config or the LEDGER_SUBSCRIPTIONS_ENABLED env var. One-way money
        ...
        # Perseus-bundle customer (0.10 = 10%). `ledger bill-savings` skips
    ...
    "alerts": {
        ...
        "smtp_port": 587,
        "smtp_user": "",
        "smtp_password": "",            # or env LEDGER_SMTP_PASSWORD
        "from_addr": "ledger@perseus.observer",
        "to_addrs": [],
    },
    "monitor": {
        # Optional bridge to the live runway monitor (repo-root ledger.py).
        # When set, the dashboard folds in live provider balances/runway.
        "enabled": False,
        "command": "",                  # e.g. "/usr/bin/python3 /opt/.../ledger.py"
    ...
    "admin": {
        ...
        # DISABLED (returns 404). Prefer the LEDGER_ADMIN_TOKEN env var over file.
        "token": "",
    },
    "ledger": {
        ...
        # the LEDGER_CHAIN_HMAC_KEY env var (preferred) — to upgrade it to a keyed
        ...
        # that old key; `ledger verify --hmac-key ...` accepts an explicit key.
        "hmac_key": "",
    },
    "pricing": {
        ...
        # individual org's ``allow_negative_balance`` (see `ledger org
...
def _load_yaml(path: Path) -> dict:
    try:
        ...
        import sys
        sys.stderr.write(f"ledger: could not read config {path}: {e}\n")
        return {}
...
def load() -> dict:
    ...
    if env.get("LEDGER_SMTP_PASSWORD"):
        cfg["alerts"]["smtp_password"] = env["LEDGER_SMTP_PASSWORD"]
    if env.get("LEDGER_ADMIN_TOKEN"):
        cfg.setdefault("admin", {})["token"] = env["LEDGER_ADMIN_TOKEN"]
    if env.get("LEDGER_PORT"):
        try:
            cfg["server"]["port"] = int(env["LEDGER_PORT"])
        except ValueError:
            pass

    # auth / OIDC overrides
    if env.get("LEDGER_AUTH_ENABLED"):
        cfg["auth"]["enabled"] = env["LEDGER_AUTH_ENABLED"].strip().lower() in (
            "1", "true", "yes", "on")
    if env.get("LEDGER_GOOGLE_CLIENT_ID"):
        cfg["auth"]["google_client_id"] = env["LEDGER_GOOGLE_CLIENT_ID"]
    if env.get("LEDGER_GOOGLE_CLIENT_SECRET"):
        cfg["auth"]["google_client_secret"] = env["LEDGER_GOOGLE_CLIENT_SECRET"]
    if env.get("LEDGER_BASE_URL"):
        base_url = env["LEDGER_BASE_URL"].rstrip("/")
    ...
    if env.get("LEDGER_ALLOWED_EMAILS"):
        cfg["auth"]["allowed_emails"] = [
            e.strip() for e in env["LEDGER_ALLOWED_EMAILS"].split(",") if e.strip()]
    if env.get("LEDGER_ALLOWED_DOMAIN"):
        cfg["auth"]["allowed_domain"] = env["LEDGER_ALLOWED_DOMAIN"]
    if env.get("LEDGER_ALLOW_SIGNUP"):
        cfg["auth"]["allow_signup"] = env["LEDGER_ALLOW_SIGNUP"].strip().lower() in (
            "1", "true", "yes", "on")
    # #175: recurring Pro/Team subscription checkout gate (default off)
    if env.get("LEDGER_SUBSCRIPTIONS_ENABLED"):
        cfg.setdefault("billing", {})["subscriptions_enabled"] = (
            env["LEDGER_SUBSCRIPTIONS_ENABLED"].strip().lower() in (
                "1", "true", "yes", "on"))
    # #108: keyed-MAC secret for the ledger tamper-evidence chain.
    if env.get("LEDGER_CHAIN_HMAC_KEY"):
        cfg.setdefault("ledger", {})["hmac_key"] = env["LEDGER_CHAIN_HMAC_KEY"]
    return cfg
...
def _strip_env_secrets(cfg: dict) -> dict:
    ...
    pairs = [
        ...
        ("alerts", "smtp_password", "LEDGER_SMTP_PASSWORD"),
        ("auth", "google_client_secret", "LEDGER_GOOGLE_CLIENT_SECRET"),
        ("admin", "token", "LEDGER_ADMIN_TOKEN"),
        ("ledger", "hmac_key", "LEDGER_CHAIN_HMAC_KEY"),
    ]
...
def ensure_initialized() -> tuple[Path, bool]:
    """Create ``~/.ledger/config.yaml`` from defaults if it doesn't exist.
