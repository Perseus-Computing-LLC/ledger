...
<title>Ledger — {org_name_escaped} — {period_label_escaped} spend report</title>
<style>
  @page {{ margin: 1.5cm; }}
...
<div class="head">
  <div><div class="brand"><span class="amber">◆</span> Ledger</div>
...
<div class="foot">Generated {gen} by Ledger v{_version()}. Costs marked estimated where exact provider cost was not supplied. · perseus.observer/ledger</div>
</body></html>"""
...
def _try_pdf(report: dict, out_path: Path) -> Optional[Path]:
    ...
    story.append(Paragraph(f"◆ Ledger — {org_name_escaped}", h))
    ...
    story.append(Paragraph(
        f"Generated by Ledger v{_version()} · perseus.observer/ledger", sub))
    doc.build(story)
    return out_path
