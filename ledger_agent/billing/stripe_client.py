...
class StripeClient:
    ...
    def _require(self):
        ...
        if self.currency != "usd":
            raise BillingError(
                f"Ledger tracks credit in USD only (billing.currency is "
        ...
        session = self._stripe.checkout.Session.create(
            ...
            line_items=[{
                "price_data": {
                    ...
                    "unit_amount": int(round(amount_usd * 100)),
                    "product_data": {
                        "name": "Ledger savings-share (voluntary)",
            ...
            success_url=self.billing.get("success_url", ""),
            cancel_url=self.billing.get("cancel_url", ""),
            metadata={"ledger_org_id": org_id, "kind": "donation",
                      "amount_usd": f"{amount_usd:.2f}"},
        )
        return {"id": session["id"], "url": session["url"]}
    ...
    def pro_checkout(self, conn, org_id: str) -> dict:
        ...
        session = self._stripe.checkout.Session.create(
            ...
            success_url=self.billing.get("success_url", ""),
            cancel_url=self.billing.get("cancel_url", ""),
            metadata={"ledger_org_id": org_id, "kind": "subscription"},
        )
        return {"id": session["id"], "url": session["url"]}
    ...
    def team_checkout(self, conn, org_id: str) -> dict:
        ...
        session = self._stripe.checkout.Session.create(
            ...
            success_url=self.billing.get("success_url", ""),
            cancel_url=self.billing.get("cancel_url", ""),
            metadata={"ledger_org_id": org_id, "kind": "team_subscription",
                      "seat_quantity": str(quantity)},
            subscription_data={"metadata": {"ledger_org_id": org_id,
    ...
    def create_savings_invoice(self, conn, org_id: str, amount_usd: float,
        ...
        self._stripe.InvoiceItem.create(
            ...
            metadata={"ledger_org_id": org_id, "kind": "savings_share",
                      "period": period_label},
