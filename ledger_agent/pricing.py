...
1. **Plan tiers** (Free / Pro / Enterprise) — what a customer pays Ledger and
   what limits apply.
2. **Provider price tables** — public per-token prices for the upstream LLM
   ...
   doesn't supply an exact ``cost_usd``. These mirror how ``ledger.py`` prefers
...
fully offline. Prices are overridable via ``~/.ledger/config.yaml`` →
...
# per-token *inference* cost. Ledger models this as a precision **multiplier** on
...
# (benchmark artifacts such as perseus-vault#630 and Ledger#131), never from
# vendor-published "1.73x / ~2x" figures. Until a tier is calibrated with a
# measured multiplier — via ``pricing.quantization`` in ``~/.ledger/config.yaml``
