...
network, safe to drop into any agent hot path:
    ...
    ledger = Meter(org="Acme Agents")          # created if it doesn't exist
    ledger.track(provider="anthropic", model="claude-opus-4-8",
                 input_tokens=1200, output_tokens=800, task_type="code_review")
    print(ledger.balance())                     # remaining prepaid credit

**Remote mode** sends each event to a hosted Ledger over ``POST /v1/usage`` with
an API key — no local database, so many machines report into one dashboard:

    ledger = Meter(remote="https://ledger.perseus.observer",
                   api_key="ledger_sk_…")       # or env LEDGER_REMOTE_URL / LEDGER_API_KEY
    ledger.track(provider="anthropic", model="claude-opus-4-8",
                 input_tokens=1200, output_tokens=800)

Remote mode is auto-detected from ``LEDGER_REMOTE_URL`` + ``LEDGER_API_KEY`` when
...
``track()`` returns the same :class:`~ledger_agent.metering.MeterResult` either
...
_USER_AGENT = f"ledger-agent/{__version__}"
...
class Meter:
    def __init__(self, org: Optional[str] = None, *, tier: str = "free",
        ...
        self.remote = (remote or os.environ.get("LEDGER_REMOTE_URL") or "").rstrip("/") or None
        self.api_key = api_key or os.environ.get("LEDGER_API_KEY")
        ...
        if self.remote:
            if not self.api_key:
                raise ValueError(
                    "remote Meter needs an api_key (or set LEDGER_API_KEY)")
            self.cfg = config or {}
            self.conn = None
            self.org_id = None
            return
    ...
    def _track_remote(self, event: dict) -> "metering.MeterResult":
        ...
        try:
            ...
            try:
                ...
                body = {}
            if e.code == 401:
                raise LedgerAuthError("Ledger rejected the API key (HTTP 401)")
            if e.code == 402:
                ...
                raise LedgerError(
                    f"Ledger ingest failed: HTTP {e.code} {body.get('error', '')}".strip())
        except urllib.error.URLError as e:
            raise LedgerError(f"could not reach Ledger at {self.remote}: {e.reason}")
