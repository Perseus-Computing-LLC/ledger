#!/usr/bin/env python3
"""
Ledger — provider credit & spend monitor.

Named for the Greek god of wealth. Ledger watches the money flowing out of
...
budgets file (ledger.budgets.json) for providers without a balance API, so
Ledger can show "remaining = budget - ledger_spend".
"""
...
HERMES_CONFIG = os.environ.get(
    "LEDGER_HERMES_CONFIG",
    "/opt/data/webui/minions-hermes-config/config.yaml")
STATE_DB = os.environ.get(
    "LEDGER_STATE_DB",
    "/opt/data/webui/minions-hermes-config/state.db")
HERE = os.path.dirname(os.path.abspath(__file__))
BUDGETS_FILE = os.environ.get("LEDGER_BUDGETS", os.path.join(HERE, "ledger.budgets.json"))
SNAPSHOT_FILE = os.environ.get("LEDGER_SNAPSHOTS", os.path.join(HERE, "ledger.snapshots.jsonl"))

DAY = 86400

# The only providers you care about. Ledger reports exactly these, in this order.
# Override with LEDGER_PROVIDERS="deepseek,anthropic,google" (comma-separated).
# Also discovers providers from ledger.budgets.json → providers section.
_FOCUS_DEFAULTS = "deepseek,anthropic,google,openai"
FOCUS_PROVIDERS = [p.strip() for p in os.environ.get(
    "LEDGER_PROVIDERS", _FOCUS_DEFAULTS).split(",") if p.strip()]

def _discover_budgets_providers():
    """Discover additional providers from ledger.budgets.json → providers dict.
...
def load_budgets():
    ...
    if os.path.exists(BUDGETS_FILE):
        try:
            ...
            sys.stderr.write(f"ledger: bad budgets file {BUDGETS_FILE}: {e}\n")
    return {}
...
def _allocate_cost(total, weights):
    ...
    ``ledger_agent.hermes.allocate_cost`` (kept inline so this standalone monitor
    needs no install)."""
    n = len(weights)
    if n == 0:
        return []
...
def _read_ledger_events(conn):
    ...
    regress. Canonical copy: ``ledger_agent.hermes.read_spend_events``."""
...
def render_cli(data, color=True):
    ...
    lines.append("  est   = estimate (budget - spend, no live API — set budgets in ledger.budgets.json)")
    return "\n".join(lines)
...
<title>Ledger — provider credit monitor</title>
<style>
...
<h1>Ledger <span>god of wealth · provider credit monitor</span></h1>
...
def calibrate(pairs):
    ...
    for pair in pairs:
        if "=" not in pair:
            sys.stderr.write(f"ledger: bad --calibrate '{pair}', want provider=balance\n")
            continue
        ...
        try:
            ...
            sys.stderr.write(f"ledger: bad balance '{val}' for {prov}\n")
            continue
        spent = 0.0
    ...
    budgets.setdefault("_comment",
        ...
        "Recalibrate with: python3 ledger.py --calibrate anthropic=NN.NN")
    ...
    if out:
        print("\nRe-run with: python3 ledger.py --calibrate " +
...
def do_alert(dry_run, channel=None):
    ...
    subject = "Ledger Low Balance Alert"
    body = ("Ledger detected the following low balance conditions:\n\n" +
            "\n".join(alert_messages) +
            "\n\nCheck `ledger` for full details.\n")
    ...
    if sent == 0 and not dry_run:
        print("No alert channels configured. Add 'alerts' to ledger.budgets.json.")

# ----------------------------------------------------------------- main ----
def _resolve_version():
    """Single-source the version from ``ledger_agent.__version__``.
    ...
    ``ledger_agent/__init__.py`` without importing it; (3) last resort a sentinel.
    """
    try:
...
def main():
    ap = argparse.ArgumentParser(description="Ledger — provider credit & spend monitor")
    ...
    if args.calibrate:
        ...
        data = collect()
        with open(os.path.join(HERE, "ledger.html"), "w", encoding='utf-8') as f:
    ...
    if args.html:
        with open(args.html, "w", encoding='utf-8') as f:
            f.write(render_html(data))
        sys.stderr.write(f"ledger: wrote {args.html}\n")
    if args.json:
