#!/usr/bin/env python3
"""Tests for Ledger — provider credit & spend monitor."""
...
# Add parent to path so we can import ledger
...
class TestDeepSeekBalance(unittest.TestCase):
    ...
    def test_api_error_returns_ok_false(self):
        ...
        with patch('ledger._get', side_effect=Exception("Connection refused")):
            result = ledger.deepseek_balance("fake-key")
    ...
    def test_happy_path(self):
        ...
        with patch('ledger._get', return_value=mock_data):
            result = ledger.deepseek_balance("fake-key")
...
class TestLedgerSpend(unittest.TestCase):
    ...
    def test_missing_db_returns_empty(self):
        ...
        result, note = ledger.ledger_spend("/nonexistent/path/state.db")
    ...
    def test_aggregates_by_provider(self):
        ...
        try:
            ...
            result, note = ledger.ledger_spend(db_path)
...
""")
        ...
        self._orig_config = os.environ.get('LEDGER_HERMES_CONFIG', '')
        self._orig_budgets = os.environ.get('LEDGER_BUDGETS', '')

    def tearDown(self):
        os.unlink(self.tmp_config.name)
        os.unlink(self.tmp_budgets.name)
        if self._orig_config:
            os.environ['LEDGER_HERMES_CONFIG'] = self._orig_config
        if self._orig_budgets:
            os.environ['LEDGER_BUDGETS'] = self._orig_budgets

    @patch('ledger.ledger_spend')
    @patch('ledger.deepseek_balance')
    def test_returns_all_focus_providers(self, mock_balance, mock_ledger):
        ...
        with patch.object(ledger, 'HERMES_CONFIG', self.tmp_config.name), \
             patch.object(ledger, 'STATE_DB', '/nonexistent/state.db'), \
             patch.object(ledger, 'BUDGETS_FILE', self.tmp_budgets.name):
            data = ledger.collect()
    ...
    def test_live_balance_populated(self, mock_balance, mock_ledger):
        ...
        with patch.object(ledger, 'HERMES_CONFIG', self.tmp_config.name), \
             patch.object(ledger, 'STATE_DB', '/nonexistent/state.db'), \
             patch.object(ledger, 'BUDGETS_FILE', self.tmp_budgets.name), \
             patch.dict(ledger.BALANCE_FETCHERS, {"deepseek": mock_balance}):
            data = ledger.collect()
...
class TestCalibrate(unittest.TestCase):
    ...
    def test_calibrate_backsolves_budget(self, mock_ledger):
        ...
        with patch.object(ledger, 'BUDGETS_FILE', self.tmp_budgets.name):
            ledger.calibrate(["anthropic=74.46"])
...
class TestRender(unittest.TestCase):
    ...
    def test_cli_no_color(self):
        ...
        output = ledger.render_cli(self.data, color=False)
    ...
    def test_html_contains_providers(self):
        ...
        output = ledger.render_html(self.data)
        self.assertIn("deepseek", output)
...
class TestFormatUsd(unittest.TestCase):
    ...
    def test_none(self):
        self.assertEqual(ledger.fmt_usd(None), "—")

    def test_zero(self):
        self.assertEqual(ledger.fmt_usd(0), "$0.00")

    def test_positive(self):
        self.assertEqual(ledger.fmt_usd(42.5), "$42.50")

    def test_large_number(self):
        self.assertEqual(ledger.fmt_usd(1234.567), "$1,234.57")
...
class TestSnapshot(unittest.TestCase):
    ...
    def test_snapshot_appends_line(self):
        ...
        with patch.object(ledger, 'SNAPSHOT_FILE', self.tmp_snapshots.name):
            result_path = ledger.snapshot(data)
