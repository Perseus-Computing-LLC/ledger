# Ledger — 12-Month Delivery Plan (Jul 2026 → Jun 2027)

You are running on DeepSeek V4 Pro xhigh. Read this fully, then work.

## Current state
- **Version:** Untagged. 629 LOC across 2 files (ledger.py 404, ledger_route.py 225).
- **Status:** Live internal tool. Not yet a public release.
- **Repo:** https://github.com/Perseus-Computing-LLC/ledger
- **Local:** /opt/data/webui/minions/.minions-data/workspace/ledger/
...
Ledger is unique: no other open-source tool combines live balance monitoring, ledger-based spend tracking, budget estimation with self-correcting calibration, and automatic model routing based on runway. LLM routers (Braintrust, Inworld, Helicone) do routing but don't track credit. AI cost observability tools (Finout, Datadog) track spend but don't route. Ledger does both in 629 lines.
...
- **Config:** Reads Hermes `config.yaml` for provider keys, `state.db` for spend ledger, `ledger.budgets.json` for no-API-provider budgets

### Balancer (ledger_route.py)
...
- Route log: `ledger.routing.jsonl` (append-only decision audit trail)
...
### CI workflow
- Lint + `python3 ledger.py --json` smoke test on push.
...
- Configurable in `ledger.budgets.json`
...
### Hardcoded model IDs → config (ledger#3)
- Move FLAGSHIP/SUBTASK dicts to `ledger.budgets.json`
...
- Data source: `ledger.snapshots.jsonl` (already being appended hourly)
...
- Policy config in `ledger.budgets.json`

### Backtest mode
- Replay `state.db` history to validate a policy before applying it live
- `python3 ledger_route.py --backtest <policy-name>`
...
2. **`open()` without `encoding='utf-8'`** — ledger#2. Fix trivially with `encoding='utf-8'`.
3. **No `gh` CLI** — GitHub ops go through REST API. Token reads from `/opt/data/webui/minions-hermes-config/cache/bws_cache.json`.
4. **Config edits are high-stakes** — the user has zero tolerance for sloppy serialization. The backup + verify + round-trip pattern in `ledger_route.py` exists for this reason.
5. **Azure is DEAD** — standing directive: "drop Azure, all-in AWS." Ledger tracks only deepseek/anthropic/google.

## Your job
Start with the 🔴 critical items. ledger#1 (tests) is the highest-impact — it gates everything else by making changes safe. Write the 7 priority tests, then tag v0.1.0, then set up CI. After that, ledger#3 (move model IDs to config) followed by the 4th provider. The architecture is solid enough to scale to v1.0 as-is — the work is execution, not redesign.
