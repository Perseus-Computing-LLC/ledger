# Ledger revamp + awareness push — kickoff prompt (2026-07)
...
If your agent has a skill registry, scan it for a `ledger` skill first. If none
exists, create one at the end capturing this revamp workflow.

## What Ledger is (context — verify, don't trust blindly)
Ledger = self-hosted, Stripe-integrated usage metering + prepaid-credit billing for
...
- Repo: https://github.com/Perseus-Computing-LLC/ledger (PRIVATE org repo; PyPI
  package `ledger-agent`).
- ~89 files. Key modules: `ledger.py` (metering/monitor + spend ledger read),
  `ledger_route.py` (runway router), `ledger_agent/` package (Meter SDK, bridge,
  cli, db, metering, alerts, client, config, demo), route/API layer, Stripe
  webhooks, `dashboard.html`, `docs/`, `tests/` (`test_ledger.py` + `tests/*`),
  `openapi.yaml`, `Dockerfile`, `docker-compose.yaml`.
- **Version discrepancy to reconcile:** `README.md`/`ROADMAP-1.0.md` claim v1.0.0
  (code-frozen; tag/publish pending), but `ledger.py --version` reports v0.1.1.
...
- Roadmaps: `ROADMAP-1.0.md` (billing-engine; has open v0.7.1 carryover items
  ...
  `prompts/ledger-roadmap-2026-2027.md` (12-month plan).
...
**Why it matters for Ledger (the headline improvement):** Ledger reads Hermes
...
which corrupts Ledger's runway projections AND its routing decisions. Consuming
...
- Inspect how Ledger reads `state.db` today (`ledger.py` spend ledger;
  `examples/hermes_sync.py`, `examples/hermes_integration.py`,
  `tests/test_hermes_sync.py`).
- Update Ledger to consume `session_model_usage` for accurate per-model /
...
- Verify runway projections and the router (`ledger_route.py`) improve under
  correct attribution. Add/extend tests. Open a PR. Run the full test suite
  (`pytest tests/ -q` plus `test_ledger.py`) and get CI green before declaring done.

## Phase 3 — Partner / deep-integration target hunt (the core ask)
Systematically find platforms where Ledger can be embedded as their billing/metering
layer OR listed as an integration on their marketplace/plugin directory. Two buckets:
1. **DEEP integration** (Ledger as the billing/metering primitive inside their
...
2. **MARKETPLACE / PLUGIN listing** (lower-touch distribution): Stripe App
   Marketplace, Vercel/Cloudflare integrations, the MCP ecosystem (Ledger as an MCP
...
gateways/routers, note where Ledger's runway-router overlaps vs complements theirs
(positioning: "billing + credit runway they don't have").

## Phase 4 — Awareness push kickoff
- Positioning one-pager committed to the repo: what Ledger is, the "we authored the
...
- Draft launch assets: README polish, a demo recording plan (`ledger demo` →
...
- End with: a crisp status, the exact outward actions left for the human, and (if
  your toolchain supports skills) a saved `ledger-revamp` skill capturing this
  workflow.
