...
Follow-up to the reconciler (#107): `plutus reconcile` trues metered (estimated)
...
the reconciler's input shape (`{provider: usd}`), so `plutus close` can run
...
- **Optional and offline-safe.** OpenAI/Anthropic use only stdlib ``urllib`` (no
  ...
  'plutus-agent[fetchers]'``); absent, its fetcher raises a clear FetchError and
...
def fetch_aws_bedrock(start_ts: float, end_ts: float, *, client=None) -> float:
    ...
    Uses ``boto3`` (extra: ``plutus-agent[fetchers]``); credentials come from the
    ...
    if client is None:
        try:
            ...
            raise FetchError("AWS Bedrock: boto3 not installed — "
                             "pip install 'plutus-agent[fetchers]'") from e
