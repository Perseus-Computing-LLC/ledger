# Plutus revamp — independent verification report (Hermes)

**Reviewer:** Hermes Agent (independent second pass)
**Date:** 2026-07-11
**Object under review:** Claude Code's 2026-07 revamp of `Perseus-Computing-LLC/plutus`
(kickoff prompt `prompts/plutus-revamp-2026-07.md`), landed as merged PR **#97** +
open docs PR **#98**.
...
| P2 Attribution | #97 consumes v17 `session_model_usage` w/ fallback | Read `plutus.py:201-320`; prefers `session_model_usage`, COALESCE-style fallback to aggregate `sessions` for pre-v17; attribution tests present and green | **Confirmed** |
...
- **Merged (#97, on `main`):** per-model spend attribution. `_provider_spend` /
  breakdown logic in `plutus.py` now prefers the Hermes schema-v17
...
- **Test suite:** I ran `pytest tests/ test_plutus.py -q` on `main` → **300 passed in
...
- `plutus_agent.__version__` → **1.0.1** (single-sourced into the wheel; correct)
- `plutus.py` line 785 → `VERSION = "0.1.1"` (**stale**, standalone monitor)
...
P3/P4/P6) and reasonably argues `plutus.py` *may* be an independently-versioned line —
...
the repo*. The moment they do, `plutus.py --version → 0.1.1` on a "v1.0.1" product
...
single-sources `plutus.py` and `openapi.yaml` from `plutus_agent.__version__` (with a
...
persistent-FS, and teardown wired). Two Plutus-specific uses, ranked by leverage:

### 6a. Fund P2 — the concurrency/load proof (highest fit, low cost)
Plutus's money-correctness story is currently **asserted, not stress-proven.** A
...
diligences. **This is the highest-ROI use of the credit for Plutus specifically.**
...
switches) that produces a real Hermes `state.db`, then show Plutus's per-provider spend
...
it needs a Plutus consumer bolted on.

### 6c. Note on scope — keep Plutus and Perseus Vault distinct
The credit has been feeding the **Perseus Vault** benchmark campaign (Gauntlet, fleet
throughput, 10k-entity recall). Plutus is a *different* product (billing layer, not
memory). Don't let the two campaigns blur: the Vault benchmarks answer "is retrieval
good and cheap"; the Plutus Lambda spend above answers "is the money ledger correct under
load." Both are legitimate, but they're separate line items against the same $7k.

**Suggested split:** cap Plutus Lambda spend at ~$200-300 (6a is cheap, 6b is a
...
- `gh repo clone Perseus-Computing-LLC/plutus` → 92 tracked files, HEAD `b568f17`
- `git log --oneline` → confirmed #97 merged, v1.0.1 lineage (#92-#96)
- `gh pr view 98` → docs-only, 6 files, +634/-0
- `python3 -c "import plutus_agent; print(plutus_agent.__version__)"` → `1.0.1`
- `grep -n VERSION plutus.py` → `0.1.1` (line 785); `openapi.yaml` → `1.0.0` (line 4)
- `grep -n session_model_usage plutus.py` → prefer-v17-with-fallback logic at 201-320
- `pytest tests/ test_plutus.py -q` → **300 passed in 21.77s**
