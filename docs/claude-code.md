...
- Attribute to a specific org: `export LEDGER_ORG="My Team"`.
- Set a monthly cap per project and get alerted before you blow it:
  `ledger workspace create my-project --budget 50`.
- Add prepaid credit so spend depletes a balance you control:
  `ledger topup --amount 25` (or buy it via Stripe — see [BILLING.md](../BILLING.md)).
...
```bash
python -m ledger_agent.integrations.claude_code_hook
```

Print the exact snippet for manual wiring with `ledger install-claude-hook --print`.

## Uninstall

Remove the `Stop` entry from `~/.claude/settings.json` (a `.ledger-bak` backup
