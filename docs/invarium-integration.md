# Invarium × Ledger: accuracy-gated, per-task savings metering

**Status:** design + working prototype (this branch). Prototype verified end-to-end
against Ledger `1.0.1` and Invarium `0.3.1`.

## Why

Ledger's differentiated revenue path is the **savings share**: bill a percentage of
...
The second half of that claim is the fragile one — Ledger meters *cost* precisely,
...
for regression detection. Its per-run unit is exactly Ledger's per-event unit.
...
| Invarium `AgentResult` | Ledger `usage_events` (`db.py`) / `record_usage` |
|---|---|
...
2. **The chain is already tamper-evident.** `usage_events` carries `prev_hash` /
   ...
   remains Ledger's own track (`feat/chain-checkpoints-independent-verify`). This
...
Forward the counterfactual baseline to Ledger **only when the task passed its
Invarium contract**. Implemented in `ledger_agent/integrations/invarium.py`:
...
`tests/test_meter_regression.py`). Ledger's cost attribution is deterministic given
...
Previously Ledger attributed by `task_type` (a category) and `workspace` (an org
...
`ledger meter --ref`, and the CSV/JSON export, and the bridge sets it from
...
2. **C (assertions + suite)** — upstream `cost_less_than`/`latency_less_than` in
   Invarium (PR #28), then a Ledger-side golden `bless`/`compare` suite that guards
   attribution.
...
- `ledger_agent/integrations/invarium.py` — the dependency-free bridge (shape B).
