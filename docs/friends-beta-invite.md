...
stacks try Plutus. They see a big efficiency number (routing + local models +
...
**Why it's risk-free (say this if they ask):** Plutus doesn't bill their LLM
spend — that's their own provider bill, which they already pay. Plutus only
*measures* it. The only money Plutus ever charges is the $20/mo floor and
...
>
> Plutus meters it and shows you: **"$X of API-equivalent value for $Y actual —
...
1. **Provision their org + key** on `plutus.perseus.observer`:
   ```
   plutus org create "<Friend Name>" --tier enterprise --email <their-email>
...
2. **They install the Claude Code plugin** (once it's on the community
   ...
   ```
   …then create `~/.claude/plutus_cc_config.json`:
   ```json
   { "remote": "https://plutus.perseus.observer", "api_key": "plutus_sk_<theirs>" }
   ```
   **Interim (before the plugin is published):** send them
   `integrations/claude-code-plugin/scripts/plutus-meter.py` plus the config
   above, and a one-line Stop hook in `~/.claude/settings.json`:
   ```json
   { "hooks": { "Stop": [ { "hooks": [
     { "type": "command", "command": "python \"~/.claude/plutus-meter.py\"" }
   ] } ] } }
   ```

3. **They keep working.** At session end the hook meters their usage.

4. **Show them the number:** `plutus efficiency --org "<Friend Name>"` →
