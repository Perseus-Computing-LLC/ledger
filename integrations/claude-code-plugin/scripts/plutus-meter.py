#!/usr/bin/env python3
"""Claude Code -> Plutus metering hook (standalone, stdlib only).
...
Plutus ``/v1/usage`` so Claude Code spend lands in the ``claude-code`` org.
...
- Idempotency-Key = transcript + line range, so a retry after a lost response
  can't double-count.
- Config from env, falling back to ~/.claude/plutus_cc_config.json:
  {"remote": "https://plutus.perseus.observer", "api_key": "plutus_sk_..."}.
...
CONFIG_PATH = Path.home() / ".claude" / "plutus_cc_config.json"
STATE_PATH = Path.home() / ".claude" / "plutus_cc_watermarks.json"
TIMEOUT = 6.0
...
def _load_config() -> dict:
    ...
    if CONFIG_PATH.exists():
        try:
            ...
            cfg = {}
    remote = os.environ.get("PLUTUS_REMOTE_URL") or cfg.get("remote") \
        or "https://plutus.perseus.observer"
    api_key = os.environ.get("PLUTUS_CC_API_KEY") or cfg.get("api_key") or ""
...
def main() -> int:
    ...
    cfg = _load_config()
    if not cfg["api_key"]:
        sys.stderr.write("[plutus] no claude-code api key configured; skipping\n")
        return 0
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            # on the public host.
            "User-Agent": "plutus-claude-code-hook",
        },
        method="POST",
    )
    try:
        ...
        sys.stderr.write(f"[plutus] claude-code meter deferred (non-fatal): {e}\n")
    return 0
